...
#
# The insulation layer proper, and the live output transform as of Phase 4. It
# supersedes the prior ScreenRepair (retired in Phase 4) whose two fatal flaws it
...
# Env flag that disables the insulation stage entirely (instant rollback to the
# pre-insulator wrapper, no code change). Default ON. Only these explicit
# off-values disable it; anything else (including unset) leaves repair enabled.
_REPAIR_OFF_VALUES = frozenset({"0", "off", "false", "no"})
# DEFAULT-ON rollout (Phase 4): the insulation layer replaced the prior ad-hoc
# ScreenRepair, which leaked private-CSI as glyphs and dropped the control plane.
# The Insulator drives the canonical VT500 parser (no byte can leak as text) and
# forwards the control plane verbatim, and the full real-terminal corpus is green
# (transparency on every correct stream incl. startup + correction on the buggy
# one — DW-4.6a). So repair is now ACTIVE by default; set CLAUDE_WRAP_REPAIR to an
# off-value below for a byte-identical rollback to FrameMux+apply_subs only.
...
class RepairState:
    """Session-scoped holder for the output-insulation pipeline so main()'s loop,
    ...
    cc-routine-and-class-design). Groups the two pure transforms (Insulator +
    FrameMux) and the two sticky booleans that gate them.

    The `sr` slot holds the Insulator (Phase 4 retired the prior ScreenRepair).
    The Insulator has the SAME public surface — feed/drain/reset/has_pending — so
    this holder, the I/O loop, the SIGWINCH closure, and the drains are unchanged
    apart from which transform `sr` is.

    enabled  — the env flag's decision, fixed for the session (default ON).
    disabled — the safety barricade tripped (an Insulator exception). Once set,
               repair is bypassed for the REST of the session: bytes flow through
               FrameMux+apply_subs exactly as the pre-insulator wrapper did, so the
               session never breaks or freezes (the plan's barricade contract).
    """
    ...
    def __init__(self, rows: int, cols: int) -> None:
        ...
        # Only build the (stateful) Insulator when repair is on; when off, the
        # output path is byte-identical to the pre-insulator mux-only wrapper.
        self.sr = Insulator(rows, cols) if self.enabled else None
        self.mux = FrameMux()
        self.disabled = False
        self._logged = False
    ...
    def _trip(self, where: str, exc: BaseException) -> None:
        """Barricade (cc-defensive-programming): disable repair for the rest of
        the session and log ONCE. This is the session-level backstop — the
        Insulator's feed/drain are already total (they never raise on malformed
        input), but a defect anywhere in the transform must degrade to raw
        passthrough, never crash the live session. Logging is best-effort — a
        failure to write the debug file must never propagate into the I/O loop."""
        self.disabled = True
        if self._logged:
            return
        self._logged = True
    ...
    def correct(self, data: bytes) -> bytes:
        """Run the insulation stage on a chunk, guarded. Returns the corrected
        bytes (claude's intended grid re-rendered absolutely, with faithful
        original SGR + the control plane forwarded verbatim) when repair is
        active; the raw bytes unchanged when repair is off/disabled. ANY exception
        from Insulator.feed trips the barricade and returns the raw bytes, so
    ...
    def on_resize(self, rows: int, cols: int) -> None:
        """SIGWINCH: track the new geometry in the Insulator's screen so post-
        resize content lands on the right rows. Insulator.reset() rebuilds the
        parser + screen + snapshot, clearing any held token, so a resize mid-
        stream can't strand a half-parsed sequence. Guarded — a reset failure
...
def process_output(data: bytes, state: "RepairState") -> bytes:
    """The master_fd -> stdout pipeline for one chunk:
        ...
        claude bytes -> Insulator (clamp-free re-render via absolute CUP, faithful
                        SGR, control plane forwarded verbatim at stream position)
                     -> FrameMux.feed (apply_subs theming, frame-bracketing)
                     -> bytes to write

    The Insulator is strictly UPSTREAM of the unchanged FrameMux (no double-
    handling): the Insulator forwards a 2026 set/reset as ordinary control raw but
    NEVER synthesizes a BSU/ESU frame, and claude emits no DEC-2026 frames in tmux,
    so FrameMux only ever sees out-of-frame bytes here and its dormant in-frame
    path stays untouched — exactly as it did under ScreenRepair. If a genuine 2026
    frame ever appeared it would still be FrameMux, the sole frame handler, that
    brackets it downstream.

    With repair OFF/disabled, `state.correct` returns `data` unchanged, so this is
    byte-identical to the pre-insulator wrapper's `mux.feed(data)` (DW-4.3)."""
...
def main() -> int:
    ...
    # Output-insulation pipeline state (Insulator + FrameMux + the flag/
    # barricade booleans), shared by the loop, the SIGWINCH closure, and the
    # drains. Sized to the current geometry; SIGWINCH re-sizes the Insulator.
