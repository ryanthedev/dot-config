...
def main():
    ...
    # 2) apply_subs output — confirm the Bash tool name recolor actually
    # fired by looking for its distinctive byte sequence (bold + orange +
    # "Bash"). This is now a literal sub, but we still verify it produces
    # the expected output against real claude bytes.
    processed = mod.apply_subs(data)
    bash_name_sig = b'\x1b[1m' + mod.PALETTE['bash'] + b'Bash'

    print("\n=== apply_subs output signatures ===")
    regex_missing = []
    ...
    if missing or regex_missing:
        ...
        if regex_missing:
            print(
                f"\nFAIL: {len(regex_missing)} signature(s) missing after "
                "apply_subs.\nThe Bash tool envelope needle in SUBS likely "
                "no longer matches claude's output.",
                file=sys.stderr,
            )
        return 1

    print("\nOK: literal SUBS produce expected output.")
    return 0
